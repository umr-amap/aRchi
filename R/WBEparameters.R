...
#' @param level characters. At which level R_ratio has to be estimated. "Node" for node level, "Axis" for branch level, "branching_order" for branch order level and "Tree" for tree level (default).
...
#' @details
#' Details for WBE parameters calculation are given in the details part of function [Make_Node()].
#' @references
#'
...
#' @seealso [Make_Node()] for node metrics estimation; [LeonardoRatio()] to estimates Leonardo Da Vinci's ratio at different level.
#' @include aRchiClass.R
#' @examples
